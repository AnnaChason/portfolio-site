...
const BookSpine: React.FC<bookSpineProps> = ({bprops, onClick, style}) => {
    ...
    return (
        <div onClick={onClick} className={(style == 'hidden' ? 'lg:hidden ' : '') + "relative flex flex-col h-[270px] w-16 rounded-sm m-2 mt-3 mb-0 justify-center items-center hover:-translate-y-2 " + ColorMap["ps"+color] +" "+ className } style={{backgroundSize:'100% 100%'}}>
...
export const BookSpineSide: React.FC<bookSpineProps> = ({bprops, onClick}) => {
    ...
    const className = bprops.className;
    return (
        <div  onClick={onClick} className={"relative flex flex-col h-16 w-[270px] rounded-md m-2 mt-0 mb-0 justify-center items-center hover:-translate-y-2 bg-no-repeat bg-center " + ColorMap["pss"+color] + " " + className}  style={{backgroundSize:'100% 100%'}}>
