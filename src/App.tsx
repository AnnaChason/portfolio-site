        ...
        <div className="w-screen min-h-screen bg-plaid flex flex-col justify-center gap-0 sm:overflow-hidden overflow-x-scroll">
            ...
            <div className="flex flex-col justify-center -translate-y-24 gap-2">
                <div className={"flex flex-row justify-center"}>
                    ...
                    <Shelf category={'Contact'}><Book onClick={() => setOpenBookId(book('contact').title)} bprops={book('contact')} ></Book></Shelf>
                </div>
                <Shelf category={'Programming Languages'}>
                    <Book bprops={book('sql')} style={"spine"} classname={" !w-12 h-56  rotate-6"}></Book>{/*SQL*/}
                    ...
                    <Book bprops={book('typescript')} style={"spine"} classname={" !h-56"} ></Book>{/*ts*/}
                    <div className={"hidden sm:block"}>
                    <Book bprops={book('javascript')} style={"spine"} classname={" !h-64"}></Book>{/*js*/}
                     </div>
                ...
                <Shelf category={'Technologies and Frameworks'}>
                    <div className={"flex lg:flex-col justify-end items-center"}>
                        ...
                        <Book bprops={book('react')} style={'spine-side'} classname={"  h-20"}></Book>{/* make blue*/}
                    </div>
                ...
                <Shelf category={'Projects'}>
                    ...
                    <Book onClick={() => setOpenBookId(book('portfolioWebsite').title)} bprops={book('portfolioWebsite')} style={"spine"} classname={" !h-52"}></Book>
                    <Book onClick={() => setOpenBookId(book('proxyWebServer').title)} bprops={book('proxyWebServer')} style={"spine"} classname={" !w-12 !h-[220px] -rotate-3"}></Book>
                ...
                <Shelf category={'Work/Volunteer Experience'}>
                    <Book onClick={() => setOpenBookId(book('littleProfessor').title)} bprops={book('littleProfessor')} classname={" !h-[240px] w-40"}></Book>
                    <div className={"flex flex-row-reverse lg:flex-col flex-wrap justify-end items-center"}>
                        <Book onClick={() => setOpenBookId(book('csClubCabinet').title)} bprops={book('csClubCabinet')} style={"spine-side"} classname={" !w-52"}></Book>
                        <Book onClick={() => setOpenBookId(book('discreteMathTa').title)} bprops={book('discreteMathTa')} style={"spine-side"} classname={" !w-60 !h-12"}></Book>
                        <Book onClick={() => setOpenBookId(book('labAssistant').title)} bprops={book('labAssistant')} style={"spine-side"} classname={" !w-60 !h-20"}></Book>
                    </div>
                    <Book onClick={() => setOpenBookId(book('diakonoi').title)} bprops={book('diakonoi')}></Book>{/*DEKES*/}
